...
class CadCAD_GPT:
    def __init__(self, api_key: str, model: Model, simulation: Simulation, experiment: Experiment, df: pd.DataFrame, docs:str = ''):
        """
    ...
    def __call__(self, user_input: str):
        ...
        if plan_list == []:
            return 'Planner Agent didn\'t make a plan. Please try again or tweak the prompt to a question.'

        # If there are plans, they are printed and executed one by one
        print(Fore.MAGENTA + Style.BRIGHT + "Planner Agent:" + Style.RESET_ALL)
